...
use starfield::jplephem::{calendar, names, spk::{SPK, seconds_to_jd}};
...
fn display_file_format(spk: &SPK) {
    ...
    print_named_value("Format", &spk.daf.locfmt);
    print_named_value("Internal name", &spk.daf.locifn);
    print_named_value("Summary records", format!("ND={}, NI={}", spk.daf.nd, spk.daf.ni));
    print_named_value("Record pointers", format!("FWARD={}, BWARD={}, FREE={}", 
...
fn display_segments(spk: &SPK) -> Result<(HashSet<i32>, HashSet<i32>)> {
    ...
    for segment in &spk.segments {
        ...
        let target_name = names::target_name(segment.target)
            .unwrap_or_else(|| "Unknown");
        let center_name = names::target_name(segment.center)
            .unwrap_or_else(|| "Unknown");
...
fn display_time_coverage(earliest_date: f64, latest_date: f64) -> Result<()> {
    ...
    print_named_value("Start date", format!("{} (JD {:.1})", earliest_iso, earliest_date));
    print_named_value("End date", format!("{} (JD {:.1})", latest_iso, latest_date));
    print_named_value("Duration", format!("{:.1} days ({:.1} years)", total_duration_days, total_duration_years));
...
fn display_debug_info(spk: &mut SPK) -> Result<()> {
    ...
    if let Ok(summaries) = spk.daf.summaries() {
        ...
        for (i, (name, values)) in summaries.iter().enumerate() {
            ...
            println!("  Seconds since J2000: start={}, end={}", start_second, end_second);
