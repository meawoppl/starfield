...
use crate::jplephem::errors::{JplephemError, Result, io_err};
...
impl DAF {
    ...
    fn initialize(&mut self) -> Result<()> {
        ...
        if self.locidw == "NAIF/DAF" || self.locidw.starts_with("DAF/") {
            ...
            self.locfmt = String::from_utf8_lossy(&file_record[88..96]).trim_end().to_string();
        ...
        if !found_valid_format {
            return Err(JplephemError::InvalidFormat(
                format!("Could not parse file as a valid DAF/SPK file. ID: {}, Format: {}",
    ...
    fn parse_file_record(&mut self, file_record: &[u8]) -> bool {
        ...
        match self.endian {
            Endian::Big => {
                ...
                self.locifn = String::from_utf8_lossy(&file_record[16..76]).trim_end().to_string();
                ...
                Ok(file_clone) => {
                    ...
                    match unsafe { MmapOptions::new().map(&file_clone) } {
                        ...
                        Err(e) => {
                            ...
                            eprintln!("Warning: Memory mapping failed: {}. Falling back to regular I/O.", e);
                        }
                    }
                },
                Err(e) => {
                    // File clone failed - log warning but continue
                    eprintln!("Warning: Could not clone file handle: {}. Falling back to regular I/O.", e);
                }
            }
        }
            ...
            for i in 0..n_summaries {
                ...
                for j in 0..self.ni as usize {
                    ...
                    let value = match self.endian {
                        ...
                        Endian::Little => LittleEndian::read_i32(&summary_data[pos..pos + 4]) as f64,
                    };
                    values.push(value);
                }
    ...
    pub fn read_array(&mut self, start: usize, end: usize) -> Result<Vec<f64>> {
        ...
        if start < 1 || end < start {
            return Err(JplephemError::InvalidFormat(
                format!("Invalid array bounds: start={}, end={}", start, end)
            ));
        }
